// Unified brand lockup: CMY circles + simfonik wordmark in a single SVG.
// Geometry is expressed entirely in SVG coordinates — no flex, no
// items-center, no optical-nudge transforms in JSX.
//
// CSS hooks (defined in globals.css):
//   .wordmark-mark    — outer span; scopes the multiply blend + hover
//   .wordmark-colors  — circles group; targeted for animation keyframes
//   .wordmark-link    — applied by parent link to gate hover animation
//
// ViewBox geometry (1350.16 × 400):
//   Circles: 400×400 region at the left; native cy values (120, 220)
//     plus VERTICAL_NUDGE so visible mass sits bbox-centered.
//   Wordmark: 925.28×262.97 native, scaled by 0.968 so its rendered
//     height is 28/44 of the circles' rendered height (so at the default
//     44px lockup render, wordmark renders at 28px tall).
//   Wordmark anchor x=454.5: gap-1.5 (6px @ 44px lockup render) to the
//     right of the circles' bbox.
//   Wordmark anchor y=45.48 + VERTICAL_NUDGE: vertically aligns the
//     wordmark's cap-to-baseline center with the circles' visual center.
interface SimfonikLockupProps {
  /** Rendered height in CSS px. Defaults to 44 (matches header). */
  height?: number;
  className?: string;
}

// All visible content is shifted down by VERTICAL_NUDGE viewBox units
// so it sits bbox-centered within VIEWBOX_HEIGHT. Without this, the
// circles' natural cy values (120, 220) leave a tall empty region at
// the bottom of the viewBox; that empty space pushes the visible
// lockup above the bbox center, so flex items-center misaligns the
// lockup visually with the header nav.
...
const VIEWBOX_WIDTH = 1350.16;
const VIEWBOX_HEIGHT = 400;
const WORDMARK_X = 454.5;
const WORDMARK_Y = 45.48 + VERTICAL_NUDGE;
const WORDMARK_SCALE = 0.968;

export function SimfonikLockup({ height = 44, className = '' }: SimfonikLockupProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <g
          transform={`translate(${WORDMARK_X} ${WORDMARK_Y}) scale(${WORDMARK_SCALE})`}
        >
